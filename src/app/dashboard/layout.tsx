...
export default function DashboardLayout({ children }: { children: React.ReactNode }) {
  ...
  return (
    <div style={{ display:'flex', minHeight:'100vh', background:'#0A0D14', color:'#E8EDF5', fontFamily:'system-ui,sans-serif' }}>
      ...
      <aside style={{ width:'220px', background:'#080B10', borderRight:'1px solid rgba(255,255,255,0.06)', display:'flex', flexDirection:'column', position:'fixed', top:0, left:0, height:'100vh', overflowY:'auto', flexShrink:0 }}>
        ...
        <div style={{ padding:'16px', borderBottom:'1px solid rgba(255,255,255,0.06)' }}>
          <Link href="/dashboard" style={{ textDecoration:'none', display:'flex', alignItems:'center', gap:'10px' }}>
            ...
            <div>
              <div style={{ fontWeight:'800', fontSize:'16px', color:'#E8EDF5', letterSpacing:'-0.5px' }}>DI<span style={{ color:'#F5A623' }}>Z</span>GO</div>
              <div style={{ fontSize:'9px', color:'#5A6478', marginTop:'1px' }}>Hallazgo de dinero</div>
            </div>
          </Link>
        </div>
        ...
        <nav style={{ flex:1, padding:'8px 6px' }}>
          {NAV.map(group => (
            <div key={group.group} style={{ marginBottom:'14px' }}>
              <div style={{ padding:'4px 10px 6px', fontSize:'9px', fontWeight:'800', letterSpacing:'1.5px', color:group.color }}>
                {group.group}
              </div>
              {group.items.map(item => {
                ...
                return (
                  <Link key={item.href} href={item.href} style={{
                    ...
                    color: active ? group.color : '#8B96A8',
        ...
        <div style={{ padding:'12px', borderTop:'1px solid rgba(255,255,255,0.06)' }}>
          ...
          <Link href="/auth/login" style={{ display:'block', textAlign:'center', padding:'7px', background:'rgba(255,255,255,0.04)', borderRadius:'8px', color:'#5A6478', textDecoration:'none', fontSize:'12px' }}>
            Cerrar sesión
          </Link>
        </div>
      </aside>
      ...
      {/* MAIN */}
      <main style={{ flex:1, marginLeft:'220px', padding:'28px', minWidth:0 }}>
        {children}
      </main>
    </div>
  )
